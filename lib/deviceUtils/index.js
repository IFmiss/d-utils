...
/**
  ...
  * DeviceUtils.initRem()
  */
export function initRem(BaseWidth = 750, MaxWidth = window.innerWidth, isFullOverMax = false) {
    const r = {};
    const MaxWidthP = MaxWidth / BaseWidth;
    r.Html = document.getElementsByTagName('html')[0];
    r.intiFontSize = function () {
        const baseOrientation = Math.min(window.innerWidth, window.innerHeight);
