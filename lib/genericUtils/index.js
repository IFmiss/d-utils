...
const index_2 = require("./../decoratorUtils/index");
/**
 * 通用工具类
 */
class GenericUtils {
     ...
     */
    static layoutFramework() {
        Array.prototype.forEach.call(document.querySelectorAll('*'), function (a) { a.style.outline = '1px solid #' + (~~(Math.random() * (1 << 24))).toString(16); });
    }
    /**
     ...
     */
    static calcStringLength(str, isStrict) {
        if (typeof str !== 'string') {
            index_1.default.logError(`str must be string but found ${typeof str}`, '[d-utils] GenericUtils calcStringLength error => ');
            return;
        }
        if (!isStrict)
            return str.length;
     ...
     */
    static strTrim(str, type = 0) {
        if (typeof str !== 'string') {
            index_1.default.logError(`str must be string but found ${typeof str}`, '[d-utils] GenericUtils strTrim error => ');
            return;
        }
        switch (type) {
            case 0:
     ...
     * }, 1000)
     */
    static throttle(fn, t = 1000) {
        if (typeof fn !== 'function') {
            index_1.default.logError(`第一个参数必须是方法`, '[d-utils] GenericUtils throttle error => ');
            return;
        }
        const _fn = fn;
        let time = null;
     ...
     * }, 1000)
     */
    static debounce(fn, t, immediate = true) {
        if (typeof fn !== 'function') {
            index_1.default.logError(`第一个参数必须是方法`, '[d-utils] GenericUtils debounce error => ');
            return;
        }
        let time;
        //  立刻执行第一次该方法
     ...
     */
    static formatDate(fmt, date = new Date()) {
...
__decorate([
    index_2.default.fnTime,
    index_2.default.log
], GenericUtils, "calcStringLength", null);
