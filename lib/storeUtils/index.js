...
class StoreUtils {
     ...
     */
    static setCookie(name, value, exp = 60 * 60 * 2 * 1000) {
        const date = new Date();
     ...
     */
    static randomDataFromArr(arr, num) {
        const newArr = Array.from(new Set(arr));
        const l = newArr.length;
        const resultArr = new Array();
        if (!(num > 0)) {
            index_1.default.logError(`数量必须大于0`, '[d-utils] StoreUtils randomDataFromArr => ');
            return;
        }
        if (newArr) {
 ...
 */
StoreUtils.dataUrlToBlob = (dataUrl) => {
